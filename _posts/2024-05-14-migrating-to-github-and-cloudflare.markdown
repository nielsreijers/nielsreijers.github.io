...
I've owned [nielsreijers.com](https://nielsreijers.com) for a long time now, but I never really did anything with it. Until recently, it only had my CV, contact information and a photo. It's registered with [gandi.net](https://www.gandi.net), which also hosted it for a fee of 10 euro per month. This always felt a little steep for my use case, but not high enough to make me look into other options. 

The trigger to finally do so was this blog. Gandi's hosting comes with [Grav](https://getgrav.org/) as a CMS. It looks fine, I really don't know it very well, but it seemed much more complicated that what I needed. I just wanted to host a simple blog and occassionaly upload a new post. Also, I want to have my site under my own source control instead of using Grav's built-in editor.


## Enter GitHub Pages and Jekyll

So instead of spending a day or two getting to know Grav and all it's options, I started to look for a simpler alternative and quickly came across GitHub sites: free, simple hosting of some static data. Exactly what I was looking for. It comes with [Jekyll](https://jekyllrb.com/), a static site generator written in Ruby. You're not tied to Jekyll, you can do basically any setup you want with GitHub actions, but for my use case Jekyll works very well.
...
You can test your site locally by running `bundle exec jekyll serve`, and... oops...
...
After that it works, and our site is available on http://localhost:4000.
...
A small word of warning: when running Jekyll locally, you do need to be aware that the version GitHub uses may not be exactly the same and has some restrictions. For example, at some point I wanted to write a little plugin in Ruby, but this wouldn't work on GitHub Pages because it doesn't allow custom code for security reasons.


## Customising the layout

I wrote my first few blog posts on https://nielsreijers.github.io, but the longer term goal was always to merge it with https://nielsreijers.com, keeping the old url, but hosting the content on GitHub Pages.

While the Jekyll's default theme was nice, I preferred the Grav theme I was using on my old site, so I started tinkering with Jekyll.

The first question was how to change a Jekyll theme? As we saw, the repository doesn't contain much, and all the styling is stored in a packaged theme.
...
In this case I had the choice of either selectively overriding parts of the theme, or copying the whole contents into my site. I opted for the latter since I felt I would be making a lot of changes, and didn't want my site to break if the underlying theme was ever updated.

`bundle info --path minima` told me the theme was stored in

```
/home/niels/.rbenv/versions/3.1.2/lib/ruby/gems/3.1.0/gems/minima-2.5.1
```

This contained just a few files:

![Minima theme contents](/assets/img/2024-05-14-minima-theme-contents.png)

That's it. This clean design really helped. I had never worked with Jekyll themes or Liquid templating before, but just looking at the contents of the theme quickly made it clear what each file did and how they fit together.

I'm not a front end developer, so getting the layout right took some time. Originally I tried copying the style sheets from the old Grav site into the Jekyll template and merging the two, but this quickly got messy since the Grav code was much more complex. So I decided to stick with the simple Jekyll theme and copy whatever I felt was necessary from the Grav theme until the result was roughly the same.
...
Just before starting this migration, I had put Cloudflare in front of nielsreijers.com. This has several advantages:
 ...
 - it improves security, protecting me from DDoS attacks and detecting bots,
 - and it has better analytics than I had before, showing me how often and from where my site is accessed.

I probably don't need these for my simple site, but it's free and I wanted to try out their service.

I have to say I'm impressed with how smooth the process is to sign up! First, you enter the url of your site on the welcome page:
...
Next, the most technical step is telling your registrar to let Cloudflare manage the DNS records for your site. The wizard gives you two urls, which you have to enter in your registrar's config as nameservers to hand over control.
...
And even for a simple site like this, the insight into my visitors is nice to have:
...
Having moved the bit of static content I had on nielsreijers.com (my CV and contact information) over to the new GitHub Pages site, the final step was to have nielsreijers.com point to the new site.
...
To use a custom domain for your GitHub Pages site, it is recommended you first [verify](https://docs.github.com/en/pages/configuring-a-custom-domain-for-your-github-pages-site/verifying-your-custom-domain-for-github-pages) the domain with GitHub to make sure only you can use it. The process is very simple: you need to prove you own the domain by adding a DNS [TXT record](https://www.cloudflare.com/learning/dns/dns-records/dns-txt-record/), basically a comment that can contain anything.

GitHub provides you with a code:

![GitHub domain verification challenge](/assets/img/2024-05-14-github-domain-verification-challenge.png)

Which I added to my DNS configuration in Cloudflare:
...
It takes a few minutes to propagate, after which the domain is verified:

![GitHub domain verified](/assets/img/2024-05-14-github-domain-verified.png)

Next, getting nielsreijers.com to point to the GitHub page, is explained very clearly and takes just [two easy steps](https://docs.github.com/en/pages/configuring-a-custom-domain-for-your-github-pages-site/managing-a-custom-domain-for-your-github-pages-site).

Similar to how we needed to change the nameservers in Gandi from Gandi's own to Cloudflare's servers to hand over control of domain, we now need to change the IP of the DNS A and AAAA (for IPv6) records from Gandi's hosting to GitHub Pages. The GitHub instructions provide a clear list of which 4 IP addresses to add:
...
The instructions at this point say it takes may time for the changes to the DNS records to propagate, and that `dig nielsreijers.com +noall +answer -t A` should show the new IP addresses after a while. But it never changed:

![dig DNS output](/assets/img/2024-05-14-dig-dns-output.png)

This puzzled me for a little until I realised Cloudflare was in front of my site, so the DNS lookup always resolves to Cloudflare. When necessary Cloudflare would pass on a request on to to Gandi, and this has now changed to GitHub Pages. But from my laptop the domain name still resolves to the same Cloudflare IP address.


## Success!

After realising this and refreshing the page, the GitHub UI shows everything has been configured correctly:
...
Nothing is running in Gandi anymore. DNS is in Cloudflare, and it points to pages hosted in GitHub. That doesn't mean I have anything against Gandi. I've been satisfied with their service for years and will happily keep paying them for my domain registration.
